...
@Stateless
public class ACMInterfacePHBean implements IACMInterfacePHBean{

	@PersistenceContext(unitName = "uy.com.amensg.logistica.persistenceUnitPH")
	private EntityManager entityManagerPH;
	
	@EJB
	private IACMInterfacePersonaPHBean iACMInterfacePersonaPHBean;
		...
		try {
			...
			ACMInterfaceEstado estado = 
				entityManagerPH.find(
					...
					Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.EnProceso"))
				);
			
			Query query = entityManagerPH.createNativeQuery(
				"SELECT m.mid AS mid, "
			...
			query.setParameter("paraProcesar", 
				Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.ParaProcesar")));
			...
			if (!resultList.isEmpty()) {
				...
				if (row[1] != null) {
					result = 
						ConfigurationPH.getInstance().getProperty("robotLogistica.prefijoTipoRegistro.Contrato")
						+ " " + row[0]
						+ " " + row[1]
						+ " " + (row[2] != null ? row[2] : "");
				} else {
					result =
						ConfigurationPH.getInstance().getProperty("robotLogistica.prefijoTipoRegistro.Prepago")
						+ " " + row[0];
				}
	...
	public String getSiguienteNumeroContratoSinProcesar() {
		...
		try {
			...
			TypedQuery<ACMInterfaceNumeroContrato> query = entityManagerPH.createQuery(
				"SELECT a"
	...
	public void remove(ACMInterfaceContrato acmInterfaceContrato) {
		try {
			...
			for (ACMInterfaceContrato acmInterfaceContratoManaged : query.getResultList()) {
				entityManagerPH.remove(acmInterfaceContratoManaged);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void remove(ACMInterfacePrepago acmInterfacePrepago) {
		try {
			...
			for (ACMInterfacePrepago acmInterfacePrepagoManaged : query.getResultList()) {
				entityManagerPH.remove(acmInterfacePrepagoManaged);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void update(ACMInterfaceContrato acmInterfaceContrato) {
		try {
			...
			ACMInterfaceEstado estado = 
				entityManagerPH.find(
					ACMInterfaceEstado.class, 
					Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.Procesado"))
				);
			...
			entityManagerPH.persist(acmInterfaceContrato);
	...
	public void update(ACMInterfacePrepago acmInterfacePrepago) {
		try {
			...
			entityManagerPH.merge(acmInterfacePrepago);
	...
	public void update(ACMInterfacePersona acmInterfacePersona, Long mid) {
		try {
			...
			ACMInterfacePersona acmInterfacePersonaManaged = 
				iACMInterfacePersonaPHBean.getByIdCliente(acmInterfacePersona.getIdCliente());
			
			Query insertPersonaQuery = entityManagerPH.createNativeQuery(
			...
			Query deleteMidPersonaQuery = entityManagerPH.createNativeQuery(
				"DELETE FROM acm_interface_mid_persona WHERE mid = :mid"
			);
			deleteMidPersonaQuery.setParameter("mid", mid);
			
			Query selectPersonaQuery = entityManagerPH.createNativeQuery(
				"SELECT p.id"
			...
			Query insertMidPersonaQuery = entityManagerPH.createNativeQuery(
	...
	public void actualizarDatosMidListaVacia(Long mid) {
		try {
			...
			ACMInterfaceEstado estado = 
				entityManagerPH.find(
					ACMInterfaceEstado.class, 
					Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.ListaVacia"))
				);
	...
	public void actualizarDatosMidListaNegra(Long mid) {
		try {
			...
			ACMInterfaceEstado estado = 
				entityManagerPH.find(
					ACMInterfaceEstado.class, 
					Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.ListaNegra"))
				);
	...
	public void actualizarDatosMidNegociando(Long mid) {
		try {
			...
			ACMInterfaceEstado estado = 
				entityManagerPH.find(
					ACMInterfaceEstado.class, 
					Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.Negociando"))
				);
	...
	public void actualizarDatosMidNoLlamar(Long mid) {
		try {
			...
			ACMInterfaceEstado estado = 
				entityManagerPH.find(
					ACMInterfaceEstado.class, 
					Long.parseLong(ConfigurationPH.getInstance().getProperty("acmInterfaceEstado.NoLlamar"))
				);
