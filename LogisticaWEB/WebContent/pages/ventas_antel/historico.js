...
function reloadData() {
	$.ajax({
		url: "/LogisticaWEB/RESTFacade/ContratoRoutingHistoryANTELREST/listByContratoId/" + id
	}).then(function(data) { 
		var registros = {
...
function inputImprimirKitOnClick() {
	window.open(
		"/LogisticaWEB/pages/contrato/contrato_kit_antel_print.jsp?m=" + __FORM_MODE_PRINT + "&cid=" + id
	);
}

function inputCancelarTramiteOnClick() {
	var contrato = {
		id: id
	};
	
	$.ajax({
		url: "/LogisticaWEB/RESTFacade/ContratoREST/cancelarTramite",
		method: "POST",
