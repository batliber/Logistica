...
function reloadData() {
	...
	$.ajax({
        url: "/LogisticaWEB/RESTFacade/ContratoREST/listNuestroCreditoContextAware",
        method: "POST",
	...
	$.ajax({
        url: "/LogisticaWEB/RESTFacade/ContratoREST/countNuestroCreditoContextAware",
        method: "POST",
        contentType: 'application/json',
        data: JSON.stringify(grid.filtroDinamico.calcularMetadataConsulta())
    }).then(function(data) {
...
function inputExportarAExcelOnClick(event, element) {
	...
	if (confirm("Se exportarán " + metadataConsulta.tamanoSubconjunto + " registros.")) {
		$.ajax({
	        url: "/LogisticaWEB/RESTFacade/ContratoREST/exportarAExcelVentasNuestroCredito",
	        method: "POST",
	        contentType: 'application/json',
