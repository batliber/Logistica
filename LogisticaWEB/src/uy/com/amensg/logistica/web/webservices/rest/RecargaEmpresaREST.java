...
@Path("/RecargaEmpresaREST")
public class RecargaEmpresaREST {
	...
	public Collection<Serializable> list(@Context HttpServletRequest request) {
		...
		try {
			...
			if ((httpSession != null) && (httpSession.getAttribute("sesion") != null)) {
				IBaseBean<RecargaEmpresa> iBaseBean = lookupBean();
				
				result = iBaseBean.list(RecargaEmpresa.class);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private IBaseBean<RecargaEmpresa> lookupBean() throws NamingException {
		...
		return (IBaseBean<RecargaEmpresa>) context.lookup(lookupName);
	}
}